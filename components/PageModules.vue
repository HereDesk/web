<template>
  <div id="testcase-modules" class="mt-5 isDisplay" :key="$route.fullPath" :class="[isShowModules ? 'pg-modules col-md-2' : 'col-md-1']">
    ...
    <div v-if="isShowModules">
    <div class="pl-5 mb-2" >
      <span class="mb-2"
        :class="{ 'el-active' : !m1_id && !m2_id }" @click="click_all_modules()">全部模块</span>
      <span>
    ...
    <ul :id="item1.m1_id" class="ul-style-none" v-for="(item1,index) in modules_list" :key="index">
      <li>
        <div class="line-height-2" :class="{ 'el-active': m1_id == item1.m1_id }" @click="click_moudle_1(item1)">
          <i class="iconfont icon-8a8a8a"
          :class="[ m1_id == item1.id ? 'icon-trigon-down' : 'icon-trigon-right' ]"></i>
          &nbsp;&nbsp;{{ item1.label }}
        ...
        <ul class="t_module_second pl-5" v-if="m1_id == item1.m1_id">
          <li v-for="(item2,index2) in item1.children" :key="index2" :id="index2" @click="click_moudle_2(item2)">
            <span :class="{ 'el-active': m2_id == item2.m2_id }">
              {{ item2.label }}
              <span class="badge_num" v-show="filter_m2_id(item2.m2_id)">
                {{ filter_m2_id(item2.m2_id) }}
              </span>
            </span>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    getModuleCount() {
      ...
      this.axios
        .get(
          "/api/pm/module/count?product_id=" +
            this.product_id +
            "&type=" +
            this.query_type
        )
